"""
a los 12 dias de nov de 2019

El loquito a decidido crear una libreria para el manejo de colores.
...
excepto el excluido
4 + se puede llmar un color parecido //def colorParecido(self, pColor): >> retorna un vector de colores parecidos
"""
...
class MagicColor:
    ...
    def colorAleatoreo(self):
        """
        Anteriormente se definio un vector de colores,
        el vector solo es un arreglo con el nombre de los colores;
        te voy retornar el nombre de cualquier color
        """
        k = random.randint(0, len(self.colores)-1)
        return self.colores[k]

    def colorAleatoreoExepto(self, colorExcluido):
        """
        Existe un vector de colores definido anteriormente,
        te voy a retornar cualquier color excepto el colorExcluido
        """
        while True:
            k = random.randint(0, len(self.colores)-1)
            if str(self.colores[k]).lower().strip() == str(colorExcluido).lower().strip():
                pass
            else:
                return self.colores[k]
